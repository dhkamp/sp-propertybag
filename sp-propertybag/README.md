...
sp-propertybag is a Node.js module that provides functions for reading from and writing to SharePoint property bags. With this module, you can easily interact with SharePoint property bags to store and retrieve key-value pairs.

## Features
- Read property values from SharePoint property bags
- Write property values to SharePoint property bags
- Supports both SharePoint Online and SharePoint On-Premises

## Installation
You can install sp-propertybag using npm:
...
**Read**
```typescript
import { ListPropertyBag } from 'sp-propertybag';
ListPropertyBag.read(list, key);
```

**Write**
```typescript
import { ListPropertyBag } from 'sp-propertybag';
ListPropertyBag.write(list, key, value);
```

### Web Property Bag

Note: For write operations to work you have to disable sharepoint onlines no script policy

**Read**
```typescript
import { WebPropertyBag } from 'sp-propertybag';
WebPropertyBag.read(web, key);
```

**Write**
```typescript
import { WebPropertyBag } from 'sp-propertybag';
WebPropertyBag.write(web, key, value);
```
